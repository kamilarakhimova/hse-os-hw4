...
int curr_observers = 0;
int sockfd;

void send_observer(char* message) {
    for (int i = 0; i < curr_observers; ++i) {
    	if (sendto(sockfd, message, strlen(message), 0, (struct sockaddr*) &observer_sockets[i], sizeof(observer_sockets[i])) < 0) {
...
int main(int argc, char* argv[]) {
	...
	if ((symbols = recvfrom(sockfd, line, 256, 0, (struct sockaddr*) &clientaddr, &clntlen)) < 0) {
        ...
        if (f == 'o') {
	    observer_sockets[curr_observers] = clientaddr;
	    if (curr_observers == MAX_OBSERVERS) {
